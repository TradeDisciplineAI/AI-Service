...
def test_win_rate_adjustment_exact_fractional_division(memory_evaluator):
    ...
    assert memory_evaluator.calculate_win_rate_adjustment(wr_4_5 - 1e-12) == 0.20  # Float precision artifact
...
def test_query_similar_setups_symbol_filtering(memory_evaluator):
    ...
    rel_setups = memory_evaluator.query_similar_setups("RELIANCE", "Symbol: RELIANCE | Action: BUY", top_k=5)
    assert len(rel_setups) == 1
    assert rel_setups[0]["symbol"] == "RELIANCE"

    # Query TCS setups
    tcs_setups = memory_evaluator.query_similar_setups("TCS", "Symbol: TCS | Action: BUY", top_k=5)
...
def test_warning_flag_multi_reason_formatting(memory_evaluator):
    ...
    assert response.warning_flag == "WARNING: Low historical win rate (20.0%); Active Discipline Risks: FOMO_ENTRY_RISK"
