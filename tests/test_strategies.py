...
def mock_technicals():
    return TechnicalIndicatorsResult(
        ...
        bollinger=BollingerBandsResult(
            upper=2470.0, middle=2440.0, lower=2410.0, bandwidth=0.024
        ),
        atr=20.0,
...
def test_mean_reversion_strategy_trigger():
    ...
    oversold_technicals = TechnicalIndicatorsResult(
        ...
        bollinger=BollingerBandsResult(
            upper=2460.0, middle=2430.0, lower=2400.0, bandwidth=0.025
        ),
        atr=18.0,
    )
...
def test_score_rag_clamping(mock_technicals):
    ...
    }  # 0.50 + 0.90 = 1.40 -> Clamped to 1.00
...
def test_conviction_score_only_payload_triggers_full_integration_mode(mock_technicals):
    ...
    result = evaluator.evaluate_all(
        mock_technicals, scan, conviction_only_sentiment, rag
    )
    assert result.confidence_mode == ConfidenceMode.FULL_INTEGRATION
    assert result.required_threshold == 0.65
    # Expected: (0.50 * 0.90) + (0.30 * 0.80) + (0.20 * 0.60) = 0.45 + 0.24 + 0.12 = 0.81
...
def test_risk_reward_always_positive():
    evaluator = StrategyEvaluator()
    # Test BUY
    sl_buy, tp_buy, rr_buy = evaluator.calculate_target_prices(
        SignalAction.BUY, 150.0, 4.0
    )
    ...
    # Test SELL
    sl_sell, tp_sell, rr_sell = evaluator.calculate_target_prices(
        SignalAction.SELL, 150.0, 4.0
    )
...
def test_invalid_atr_and_prices():
    ...
    with pytest.raises(
        ValueError, match="Calculated risk distance must be greater than zero"
    ):
