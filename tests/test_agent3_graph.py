...
from src.schemas import TradeSignal, SignalAction, ConfidenceMode
...
def sample_market_scan():
    return {
        ...
        "volume_surge": True,
        "candles": [
            {"timestamp": f"2026-08-14T10:{i//60:02d}:{i%60:02d}Z", "open": 100.0 + i, "high": 102.0 + i, "low": 98.0 + i, "close": 101.0 + i, "volume": 1000 + i}
            for i in range(25)
        ]
    }
...
@pytest.mark.asyncio
async def test_agent3_graph_full_integration_mode(sample_market_scan, sample_sentiment, sample_rag):
...
def test_agent3_router_evaluate_endpoint(sample_market_scan, sample_sentiment, sample_rag):
