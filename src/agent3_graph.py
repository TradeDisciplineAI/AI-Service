...
def ingest_inputs_node(state: Agent3State) -> Agent3State:
    ...
    if not has_candles:
        try:
            ...
            logger.info(
                f"Market scan candles missing for {ticker}. Running fetch_market_data fallback."
            )
            ...
            if isinstance(fetched_data, list):
                ...
                raw_candles = (
                    fetched_data.get("ohlcv_candles")
                    or fetched_data.get("candles")
                    or []
                )
    ...
    return {**state, "market_scan_json": market_scan, "errors": errors}
...
def compute_technicals_node(state: Agent3State) -> Agent3State:
    ...
    try:
        ...
        if technicals.summary.get("insufficient_candles"):
            errors.append(
                f"Insufficient candles ({len(candles)}) for technical indicators calculation."
            )
    except Exception as e:
    ...
    return {**state, "technicals_json": technicals_dict, "errors": errors}
...
def evaluate_strategies_node(state: Agent3State) -> Agent3State:
    ...
    return {**state, "final_trade_signal": trade_signal_dict, "errors": errors}
