...
from src.config import IndicatorSettings, indicator_settings
...
def _cached_indicator_calc(
    ...
    if HAS_PANDAS_TA and len(df) >= rsi_period:
        try:
            ...
            rsi_val = (
                ...
                else calculate_rsi(df["close"], rsi_period)
            )
        except Exception:
    ...
    if ema_9_val > ema_21_val:
        trend = (
            "BULLISH_CROSS"
            if len(df) > 1 and df["close"].iloc[-2] <= ema_21_val
            else "UPTREND"
        )
    ...
    middle_band = float(
        df["close"].rolling(window=period, min_periods=1).mean().iloc[-1]
    )
    ...
    bandwidth = (
        float((upper_band - lower_band) / middle_band) if middle_band > 0 else 0.0
    )
...
def compute_technical_indicators(
    symbol: str, candles: list[dict[str, Any]], config: IndicatorSettings | None = None
) -> TechnicalIndicatorsResult:
    """
    ...
    if not candles or len(candles) < 5:
        logger.warning(
            f"Insufficient candles provided for {symbol}: {len(candles) if candles else 0} (minimum required: 5)"
        )
        ...
        fallback_dict = {
            ...
            "ema": EMAResult(
                ema_9=float(last_price), ema_21=float(last_price), trend="NEUTRAL"
            ),
            "bollinger": BollingerBandsResult(
                upper=float(last_price * 1.02),
                middle=float(last_price),
                lower=float(last_price * 0.98),
                bandwidth=0.04,
            ),
            "atr": max(round(float(last_price) * 0.001, 2), 0.01),
            "summary": {
                "error": "Insufficient candle data for calculation",
    ...
    if "timestamp" in df.columns:
        ...
        if not df["dt"].is_monotonic_increasing:
            logger.warning(
                f"Candles for {symbol} were out of order. Re-sorting chronologically."
            )
    ...
    last_timestamp = (
        str(df["timestamp"].iloc[-1]) if "timestamp" in df.columns else str(len(df))
    )
