...
from src.database import AIAnalysis, MarketSignals, SessionLocal, StockNews

# Import our schema (the blueprint) from schemas.py
from src.schemas import BatchAnalyzeRequest
...
async def get_ai_usage(db: Session = Depends(get_db)):
    ...
    try:
        ...
        news_count = (
            db.query(AIAnalysis)
            .filter(func.date(AIAnalysis.created_at) == today)
            .count()
        )
        ...
        market_count = (
            ...
            .filter(func.date(MarketSignals.updated_at) == today)
            .count()
        )
        ...
        raise HTTPException(status_code=500, detail="Failed to check AI usage") from e


@router.post("/scan-batch")
async def scan_market_batch(
    request: BatchAnalyzeRequest, db: Session = Depends(get_db)
):
    try:
        tickers = [t.upper() for t in request.tickers]
        logger.info(
            f"Received batch request for Agent 1 to scan {len(tickers)} tickers"
        )
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            future_to_ticker = {
                executor.submit(fetch_market_data, ticker): ticker for ticker in tickers
            }
        ...
        if not valid_tickers:
            return {
                "status": "error",
                "message": "Failed to fetch market data for any tickers.",
            }
        ...
        for attempt in range(6):
            try:
                ...
                if "429" in str(e) or "RESOURCE_EXHAUSTED" in str(e):
                    logger.warning(
                        f"Agent 1 Batch: Google Rate Limit hit! Waiting 65s (Attempt {attempt + 1}/6)..."
                    )
        ...
        return {
            "status": "success",
            "message": f"Saved {saved_count} tickers to database!",
        }
...
async def analyze_stock_batch(
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
            ...
            future_to_ticker = {
                executor.submit(fetch_all, ticker): ticker for ticker in tickers
            }
            for future in concurrent.futures.as_completed(future_to_ticker):
                ticker = future_to_ticker[future]
                try:
        ...
        if not valid_tickers:
            return {
                "status": "error",
                "message": "Failed to fetch news data for any tickers.",
            }

        # 3. Call AI
        llm = ChatGoogleGenerativeAI(
            model="gemini-2.5-flash", temperature=0.1, max_retries=6
        )
        ...
        for attempt in range(6):
            try:
                ...
                if "429" in str(e) or "RESOURCE_EXHAUSTED" in str(e):
                    logger.warning(
                        f"Agent 2 Batch: Google Rate Limit hit! Waiting 65s (Attempt {attempt + 1}/6)..."
                    )
                    await run_in_threadpool(time.sleep, 65)
                    if attempt == 5:
                        raise e
        ...
        for ticker in valid_tickers:
            ...
            latest_analysis = (
                db.query(AIAnalysis)
                .filter(AIAnalysis.ticker == ticker)
            ...
            if is_new_news:
                ...
                latest_analysis.created_at = datetime.now(UTC)

            existing_news = (
                db.query(StockNews).filter(StockNews.ticker == ticker).first()
            )
            if existing_news:
                existing_news.headlines = top_headlines
                existing_news.created_at = datetime.now(UTC)
            else:
        ...
        return {
            "status": "success",
            "message": f"Saved {saved_count} tickers news to database!",
        }
...
async def get_stock_history(
    ...
    try:
        ...
        history = (
            db.query(AIAnalysis)
            .filter(AIAnalysis.ticker == ticker.upper())
            .order_by(AIAnalysis.created_at.desc())
            .all()
        )
        ...
        if not history:
            return {
                "message": f"No historical data found for {ticker.upper()}",
                "data": [],
            }
...
async def get_stock_news(
    ...
    try:
        news_record = (
            db.query(StockNews).filter(StockNews.ticker == ticker.upper()).first()
        )
...
def get_market_signals(ticker: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        signal = (
            db.query(MarketSignals)
            .filter(MarketSignals.ticker == ticker.upper())
            .first()
        )

        if not signal:
            raise HTTPException(
                status_code=404, detail=f"No market signals found for {ticker}"
            )
