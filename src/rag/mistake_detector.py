...
def parse_utc_timestamp(ts: str) -> datetime:
    ...
    try:
        ...
        logger.warning(f"Error parsing timestamp '{ts}': {e}. Falling back to current UTC time.")
        return datetime.now(timezone.utc)


def detect_discipline_mistakes(request: RAGQueryRequest, recent_trades: List[TradeExecutionRecord]) -> List[str]:
    """
    ...
    for trade in recent_trades:
        if trade.pnl < 0:
            ...
            if 0 <= time_diff <= 900:  # Within 15 minutes (900 seconds)
                ...
                logger.warning(f"REVENGE_TRADING_RISK flagged: Loss trade '{trade.trade_id}' occurred {time_diff:.1f}s ago.")
                break
    ...
    if request.rsi > 70.0 and request.price_change_pct_24h > 3.0:
        ...
        logger.warning(f"FOMO_ENTRY_RISK flagged for {request.symbol}: RSI is {request.rsi} with +{request.price_change_pct_24h}% surge.")
    ...
    if trades_in_last_hour > 5:
        ...
        logger.warning(f"OVERTRADING_RISK flagged: {trades_in_last_hour} trades executed in the last 60 minutes (max allowed: 5).")
